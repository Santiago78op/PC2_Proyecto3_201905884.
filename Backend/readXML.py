...
class XML():
    ...
    def algoritmo(self):
        ...
        for dato in self.list_data: 
            ...
            comments_nulls = 0
            # ? total de mensajes en todos los comentarios
            ...
            # ? Número total de mensajes que mencionan a Empresa     
            ...
            for empresas in company:
                ...
                for dato_ent in self.list_data:
                    for text_ent in dato_ent.comentarios:
                        ditc_cont_ent = {empresa:0}
                        if dato.fecha == dato_ent.fecha:
                            x = re.findall('\\b(' + empresa + ')\\b',text_ent)
                            ditc_cont_ent = {empresa:len(x)}
                            if ditc_cont_ent[empresa] > 0:
                                response_positive_ent = []
                                response_negative_ent = []
                                for palabra in positive_emotions:
                                    x = re.findall('\\b(' + palabra + ')\\b',text_ent)
                ...
                # ? Número total de mensajes que mencionan al servicio    
    ...
    def createXML(self,intrucciones):
        ...
        my = str(my)
        file = open("../Frontend/media/documents/salida.xml", 'w')
        file.write(my)
    ...
    def prueba(self,text):
        entrada = ''
        entrada += '<?xml version="1.0"?>'
        entrada += '<data>'
        entrada += text
        entrada += '</data>'
        with open("../Frontend/media/documents/probar.xml", 'w', encoding="utf-8") as f:
            f.write(entrada)
            f.close
        root = '../Frontend/media/documents/probar.xml'
        return  root
    
    def readXMLPrueba(self, root):
        probar = open(root, encoding="utf-8")
        tree = ET.parse(probar)
        root = tree.getroot()
        ...
        comment = ''
        list_mgs = list() 
        for respuestas in root.findall('mensaje'):
            mensaje = respuestas.text.strip()
            mensaje = self.deletTilde(mensaje)
            list_mgs.append({'mensaje':mensaje})
        dict_respuestas['mensajes'] = list_mgs
    ...
    def reserverPrueba(self,diccionario):
        lista_reservados = list()
        messages = diccionario['mensajes']
        for msg in messages:
            message = msg['mensaje']
            mjs_lugar = re.search('[L|l][A-Za-z0-9]*[\s]*[Y|y][\s]*[F|f][A-Za-z0-9]*[\s]*:[\s]*[A-Za-z0-9]*,',message)
            ...
            fecha = mjs_fecha.group()
            fecha = fecha.strip()
            lista_reservados.append({'fechga':fecha})
            message = re.sub('(\d+/\d+/\d+)','',message)
            mjs_hora = re.search('([01]?[0-9]|2[0-3]):[0-5][0-9]',message)
            hora = mjs_hora.group()
            hora = hora.strip()
            ...
            usuario = re.sub('[U|u][A-Za-z0-9]*[\s]*:[\s]*','',usuario)
            usuario = usuario.strip()
            lista_reservados.append({'usuario':usuario})
            message = re.sub('[U|u][A-Za-z0-9]*[\s]*:[\s]*[\s]*[A-Za-z0-9\S*]*|@([A-Za-z]*.)*','',message)
            mjs_red = re.search('[R|r][E|e][A-Za-z][\s]*[A-Za-z]*[\s]*:[\s]*[A-Za-z]*',message)
            red_social = mjs_red.group()
            red_social = re.sub('[R|r][E|e][A-Za-z][\s]*[A-Za-z]*[\s]*:[\s]*:','',red_social)
            red_social = red_social.strip()
            social = re.sub("red social:","",red_social)
            lista_reservados.append({'redSocial':social})
            message = re.sub('[R|r][E|e][A-Za-z][\s]*[A-Za-z]*[\s]*:[\s]*[A-Za-z]*','',message)
            mjs_comment = re.search('[A-Za-z0-9\S|\s]*',message)
            comment = mjs_comment.group()
            comment = comment.strip()
            lista_reservados.append({'comentario':comment})
            message = re.sub('[A-Za-z0-9\S|\s]*','',message)
            
        return lista_reservados

    def algoritmoPrueba(self,datos,resp):
        positive_emotions = datos['positivos']
        negative_emotions = datos['negativos']
        company = datos['empresas']
        ...
        for empresas in company:
            ...
            for servicios in empresas['servicio']:
                for service in servicios:
                    ...
                    text = coment['comentario']
                    ditc_cont_sr = {servicio:0}
                    x = re.findall('\\b(' + servicio + ')\\b',text)
                    ditc_cont_sr = {servicio:len(x)}
                    if len(alias) != 0:
                        for word in alias:
                            if word in text:
                                x = re.findall('\\b(' + word + ')\\b',text)
                                ditc_cont_sr = {servicio:len(x)}
                                if ditc_cont_sr[servicio] > 0:
                                    response_positive_sr = []
                                    response_negative_sr = []
        ...
        return lista_empresas_comments